import loadTemplate from './loadTemplate.js';

const templatePromise = loadTemplate('./cartTemplate.html');

class ShoppingCart extends HTMLElement {
    #items = [];

    constructor() {
        super();
        this.attachShadow({ mode: 'open' });
    }

    async connectedCallback() {
        if (!this.shadowRoot.hasChildNodes()) {
            const tpl = await templatePromise;
            this.shadowRoot.appendChild(tpl.content.cloneNode(true));

            this.shadowRoot.addEventListener('click', (e) => {
                const btn = e.target.closest('button[data-remove-id]');
                if (!btn) return;
                this.removeItem(btn.dataset.removeId);
            });
        }

        this.render();
    }

    addItem(product) {
        const cartItem = {
            cartItemId: crypto.randomUUID(),
            product,
        };
        this.#items = [...this.#items, cartItem];
        this.render();
    }

    removeItem(cartItemId) {
        this.#items = this.#items.filter(
            (x) => String(x.cartItemId) !== String(cartItemId)
        );
        this.render();
    }

    render() {
        if (!this.shadowRoot || !this.shadowRoot.hasChildNodes()) return;

        const itemsEl = this.shadowRoot.querySelector('.items');
        const sumEl = this.shadowRoot.querySelector('.sum');
        if (!itemsEl || !sumEl) return;

        itemsEl.innerHTML = '';

        let sum = 0;
        for (const item of this.#items) {
            const p = item.product;
            const price = Number(p.price) || 0;
            sum += price;

            const row = document.createElement('div');
            row.className = 'item';
            row.innerHTML = `
        <div>
          <div class="name">${p.name ?? ''}</div>
          <div class="price">${price.toFixed(2)} zł</div>
        </div>
        <button class="remove" type="button" aria-label="Usuń" data-remove-id="${
            item.cartItemId
        }"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
        <path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5M11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47M8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5"/>
        </svg></button>
      `;
            itemsEl.appendChild(row);
        }

        sumEl.textContent = `💰 Razem: ${sum.toFixed(2)} zł`;
    }
}

customElements.define('shopping-cart', ShoppingCart);
